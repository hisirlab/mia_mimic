...
weightPath = r"C:\Users\DELL\Desktop\AMIA\model_2\inceptionv3\H1111_model_Inceptionv3.pth"
...
if __name__ == '__main__':
    ...
    model = build_model_inceptionv3().to(device)
    criterion = nn.BCELoss()
    optimizer = optim.Adam(model.parameters(), lr=1e-4)

    save_dir = r"D:\Codes\PycharmProjects\amia\results_2\Inceptionv3\H1_11"
    ...
    print(classification_report(test_labels, pred_labels, target_names=['Tuberculosis', 'Normal'], digits=4))
    ...
    new_test_dir =  r"C:\Users\DELL\Desktop\AMIA\TB_Chest_Radiography_Database\MIA_2\2_Split\test"

    new_test_paths, new_test_labels = load_data_from_dir(new_test_dir)
    new_test_dataset = CustomDataset(new_test_paths, new_test_labels, transform=transform)
    new_test_loader = DataLoader(new_test_dataset, batch_size=32, shuffle=False)

    new_test_labels, new_pred_labels, new_predictions = evaluate_model(model, new_test_loader, device)

    print(classification_report(new_test_labels, new_pred_labels, target_names=['Tuberculosis', 'Normal'], digits=4))

    new_cm = confusion_matrix(new_test_labels, new_pred_labels)
    print(new_cm)

    new_save_dir = os.path.join("D:/Codes/PycharmProjects/amia/results_2/Inceptionv3", "H1_22")
    os.makedirs(new_save_dir, exist_ok=True)

    plot_roc_curve(new_test_labels, new_predictions, os.path.join(new_save_dir, "new_roc_curve.png"))
    plot_confusion_matrix(new_cm, ['Tuberculosis', 'Normal'], os.path.join(new_save_dir, "new_confusion_matrix.png"))

    new_test_dir_2 =  r"C:\Users\DELL\Desktop\AMIA\TB_Chest_Radiography_Database\MIA_2\3_Split\test"

    new_test_paths_2, new_test_labels_2 = load_data_from_dir(new_test_dir_2)
    new_test_dataset_2 = CustomDataset(new_test_paths_2, new_test_labels_2, transform=transform)
    new_test_loader_2 = DataLoader(new_test_dataset_2, batch_size=32, shuffle=False)

    new_test_labels_2, new_pred_labels_2, new_predictions_2 = evaluate_model(model, new_test_loader_2, device)

    print(classification_report(new_test_labels_2, new_pred_labels_2, target_names=['Tuberculosis', 'Normal'], digits=4))

    new_cm_2 = confusion_matrix(new_test_labels_2, new_pred_labels_2)
    print(new_cm_2)

    new_save_dir_2 = os.path.join("D:/Codes/PycharmProjects/amia/results_2/Inceptionv3", "H1_33")
    os.makedirs(new_save_dir_2, exist_ok=True)

    plot_roc_curve(new_test_labels_2, new_predictions_2, os.path.join(new_save_dir_2, "new_roc_curve.png"))
    plot_confusion_matrix(new_cm_2, ['Tuberculosis', 'Normal'], os.path.join(new_save_dir_2, "new_confusion_matrix.png"))